...
The box plots for blood pressure in the first and second assessment center around the value of 100 for both low and high assessments. In the third assessment, however, the blood pressures for low and high outputs are vastly different, where the values are centered around 50 for individuals with low readings, and values are centered around 130 for those with high readings.
While there are no outliers in any of the assessments for blood pressure, larger/smaller values from the median skew the variance; this is noticable in the first and second assessment in blood pressure, where the first and third quartile are far from the median. The variance of the final decision is significantly less.
...
NA values are common in data sets. I decided to handle the NA value here by replacing it with the most common number in its vector due to time. However, this practice is not recommended in real-life data sets where there are hundreds or thousands of NA values. The best practices are to analyze patterns in other columns that occur with NA values, and then handle them based on those findings. 